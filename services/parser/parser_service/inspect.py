...
citations (precision) and dropped facts (recall) at a glance:
    ...
    red    = a numeric-looking token detected on the page that produced NO
             citable fact -- most commonly DS-W3-3's ambiguity fail-close (the
...
machinery a real claim would use, inferring value_type purely from the
token's own shape (a trailing "%" -> percent, everything else -> currency).
...
misses in noise, which is exactly what this tool exists to prevent.
