...
**UFW regression found and fixed, 2026-07-28 (same day, next droplet):** on the very next fresh droplet built with the above fix, SSH became completely unreachable - not rate-limited, fully blocked, confirmed via a first-ever connection attempt from a brand new source IP (Vansh's own laptop, never rate-limit-eligible) also hanging. `cloud-init status --long` showed `status: done`, `errors: []` - cloud-init itself completed without error. `sudo ufw status verbose` on the droplet (via console, since SSH was down) revealed the actual problem: UFW active with its default `deny (incoming)` policy and **zero rules** - the `ufw --force delete limit 22/tcp` + `ufw --force allow 22/tcp` sequence had somehow resulted in no surviving allow rule for port 22, blocking everything inbound including 80/443 (which, separately, had never been explicitly opened in UFW at all before this - the marketplace image's stock config only ever covered port 22 and the Docker API ports, so Caddy's ports were never actually reachable at the UFW layer on any droplet built this session, independent of the DNS/Host-header fixes). Exact root cause of the delete-then-allow sequence not fully applying wasn't conclusively identified. Fixed defensively rather than by chasing the exact mechanism: reordered `runcmd` to `ufw allow 22/tcp` + `ufw allow 80/tcp` + `ufw allow 443/tcp` **before** any `ufw delete` calls, so a failure in the cleanup steps leaves the droplet over-permissive and reachable rather than under-permissive and locked out. Also added a trailing `ufw status verbose` to `runcmd` so the actual resulting rule state is captured in `/var/log/cloud-init-output.log` on every future boot, catching a repeat of this class of issue without needing to manually check.

**Stale hardcoded GHCR owner found and fixed, 2026-07-28:** `deploy.yml`'s `deploy` job repeatedly failed with `ghcr.io/digitallick/simpero-ai-gov-alpha:<sha>: not found`, even immediately after a confirmed-successful, fresh `docker-publish` push for that exact SHA. Root cause, confirmed directly from the `docker-publish` job's build log: the repo had been transferred from the `Digitallick` personal account to the `Simpero-AI` organization (the "This repository moved" message on every `git push` was the same signal, just not connected to this symptom until now) - `docker-publish` correctly computes `github.repository_owner` dynamically (now `Simpero-AI`, lowercased to `simpero-ai`) and pushed there, but `docker-compose.prod.yml`'s `image:` line **hardcoded** the old `digitallick` owner, so `deploy`'s pull looked in a completely different (empty) registry path. Fixed by updating the hardcoded owner in `docker-compose.prod.yml` to `simpero-ai`, and correcting a now-stale comment in `deploy.yml` that referenced the old owner name as if it were still current. Deliberately kept static rather than switched to a dynamic value: a real repo-owner change (an org transfer, as just happened) needs a deliberate look at both this file and `deploy.yml`'s image-name computation anyway, not a silent auto-follow that could mask the same class of mismatch happening again in some other form.
