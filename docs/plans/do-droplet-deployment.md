...
## 7. What's already implemented vs. what this revision changes

The **original single-droplet version** of this pipeline is implemented today: `.github/workflows/ci.yml` (with `workflow_call` added), `.github/workflows/deploy.yml` (single-environment job graph), `terraform/{versions,variables,main,outputs}.tf` + `cloud-init.yaml.tpl` (one droplet, one bucket, one `.tfvars`-free variable set), `docker-compose.prod.yml`, `Caddyfile` (hardcoded placeholder hostname).

This revision (staging/production, shared buckets) has **not yet been implemented** — it supersedes the single-bucket/single-droplet Terraform config and the single-environment `deploy.yml` job graph described in earlier versions of this document. Implementation is the next step, to be handed to the `implementer` agent once this doc is the agreed source of truth.
