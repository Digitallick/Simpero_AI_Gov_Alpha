...
A `workflow_dispatch`-only workflow, `.github/workflows/destroy.yml`, added after staging's first droplet needed a clean recreation (a corrupted `TF_VAR_ssh_public_key` value — fixed via `gh secret set < file` to avoid terminal/clipboard corruption — had produced a droplet whose cloud-init failed to parse, per a `#x0080` invalid-character YAML error, leaving the `deploy` user never created).
