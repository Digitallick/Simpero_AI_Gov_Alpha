terraform {
  # >= 1.11.0 (bumped from >= 1.5): needed for the `endpoints` block syntax
  # below and for `use_lockfile` (native S3-backend locking, GA in 1.11).
  ...
  # themselves are shared with the frontend repo, created manually, once,
  # outside any Terraform state (see docs/plans/do-droplet-deployment.md §1).
  #
  ...
  backend "s3" {
    ...
    use_path_style              = true
    # Native S3-backend state locking (GA since 1.11) — two repos now share
